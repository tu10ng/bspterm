//! Provides constructs for the Bspterm app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("BSPTERM_RELEASE_CHANNEL")
            .unwrap_or_else(|_| include_str!("../../bspterm/RELEASE_CHANNEL").trim().to_string())
    } else {
        include_str!("../../bspterm/RELEASE_CHANNEL").trim().to_string()
    }
});
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Bspterm-Dev",
        ReleaseChannel::Nightly => "Bspterm-Nightly",
        ReleaseChannel::Preview => "Bspterm-Preview",
        ReleaseChannel::Stable => "Bspterm-Stable",
    }
}

/// The Git commit SHA that Bspterm was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("BSPTERM_APP_VERSION") {
            from_env.parse().expect("invalid BSPTERM_APP_VERSION")
        } else {
...
/// A Bspterm release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Bspterm.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Bspterm on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.bspterm.Bspterm-Dev",
            ReleaseChannel::Nightly => "dev.bspterm.Bspterm-Nightly",
            ReleaseChannel::Preview => "dev.bspterm.Bspterm-Preview",
            ReleaseChannel::Stable => "dev.bspterm.Bspterm",
        }
    }
