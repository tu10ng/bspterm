...
static BSPTERM_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("BSPTERM_CLIENT_CHECKSUM_SEED")
        .map(|s| s.as_bytes().into())
        .or_else(|| {
            env::var("BSPTERM_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
impl Telemetry {
    ...
    pub fn has_checksum_seed(&self) -> bool {
        BSPTERM_CLIENT_CHECKSUM_SEED.is_some()
    }
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let checksum_seed = BSPTERM_CLIENT_CHECKSUM_SEED.as_ref()?;
